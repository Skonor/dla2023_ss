...
class MixtureGenerator:
    def __init__(self, speakers_files, out_folder, nfiles=5000, test=False, randomState=42):
        ...
        for dir in ['mix', 'refs', 'targets']:
            if not os.path.exists(os.path.join(self.out_folder, dir)):
                os.makedirs(os.path.join(self.out_folder, dir))
...
def main(num_speakers, train_nfiles, val_nfiles):
    ...
    path_mixture_val = 'data/datasets/LibriMixes/val'
    path_mixture_train = 'data/datasets/LibriMixes/train'

    speakersVal = [el.name for el in os.scandir(path_val)]
    speakersTrain = [el.name for el in os.scandir(path_train)]

    speakersTrain = np.random.choice(speakersTrain, size=num_speakers, replace=False)
    ...
    mixer_train.generate_mixes(snr_levels=[0, 0],
                               num_workers=2,
                               update_steps=100,
                               trim_db=20,
                               vad_db=None,
...
if __name__ == '__main__':
    main(num_speakers=100, train_nfiles=10000, val_nfiles=1000)